...
// ---ARKA begin
// PHProxy 0.5b2 (5:27 PM 1/20/2007) modified by Arkados on 2010-08-25. PHP editing with Notepad++. Feels good man.
// 
// French translation ! Oui monsieur !
// Also, no ROT13 anymore (does avoid Sebsauvage's session cookie protection)
// Some bold text, user to make right choice for better privacy.
// "Bookmarks" changed. Cookie manager out, not implemented and quite useless there (browser cares, user may use private browsing).
// Point creator URL to SourceForge project(original site hijacked ? Down anyway.)
//    
// 
// ---ARKA end
...
$_labels            = array
                    (
                        'include_form'    => array('Formulaire', 'Inclure un champ de mini URL sur chaque page'), 
                        'remove_scripts'  => array('Enlever scripts', '<b>Bloquer les scripts client</b> (ex: Javascript) [recommand&eacute]'), 
                        'accept_cookies'  => array('Accepter Cookies', 'Accepter le stockage de cookies'), 
                        'show_images'     => array('Afficher Images', 'Afficher les images sur les pages (et les pubs avec, animations GIF)'), 
                        'show_referer'    => array('Montrer R&eacute;f&eacute;rant', 'Afficher le r&eacute;f&eacute;rant'), 
                        'rotate13'        => array('ROT13', 'Utiliser encodage ROT13 sur URL'), 
                        'base64_encode'   => array('Base64', '<b>Utiliser encodage Base64 sur URL</b> [recommand&eacute]'), 
                        'strip_meta'      => array('Retirer Meta', 'Retirer les meta-tags des pages'), 
                        'strip_title'     => array('Retirer Titre', 'Retirer le titre de la page'), 
                        'session_cookies' => array('Cookies de session', '<b>Stocker les cookies pour cette session seulement</b> [recommand&eacute]') 
                    );
...
else
{
    ...
    if ($_flags['include_form'] && !isset($_GET['nf']))
    {
        $_url_form      = '<div style="width:100%;margin:0;text-align:center;border-bottom:1px solid #725554;color:#000000;background-color:#F2FDF3;font-size:12px;font-weight:bold;font-family:Bitstream Vera Sans,arial,sans-serif;padding:4px;">'
                        ...
                        . ' <label for="____' . $_config['url_var_name'] . '"><a href="' . $_url . '">Adresse URL</a>:</label> <input id="____' . $_config['url_var_name'] . '" type="text" size="80" name="' . $_config['url_var_name'] . '" value="' . $_url . '" />'
                        . ' <input type="submit" name="go" value="Aller" />'
                        . ' [Aller: <a href="' . $_script_url . '?' . $_config['url_var_name'] . '=' . encode_url($_url_parts['prev_dir']) .' ">R&eacute;pertoire parent</a> | <a href="' . $_script_base . '">Retour page principale</a>]'
                        . '<br /><hr />';
